...
## MAP REBUILD — code-generated floating arena (2026-09-16)

The old hand-made world is replaced by a clean cartoon floating arena:
central spawn hub, 8 floating plot islands on a causeway, market / heist /
event districts, 5 mini-biomes with nests + guardians, bridges, clouds, and
34 readable signs. Zero Lua changes — every world path the code looks up is
preserved, so the game boots and plays exactly as before.

- NEW `tools/build_world.py`: generates `assets/world/EggHeistWorld.rbxm`
  (585 instances) with standard binary encoding throughout (CFrame rotIds +
  planar positions, zigzag ints, plain-u32 enums — verified against the
  Roblox format spec), so Studio parses every property. Self-verifies.
- NEW `tools/check_map.py`: layout guards on REAL decoded positions (walk
  tops at y=0, bridge joints, NPC footing, runtime statue/beach/grotto/
  volcano/extraction zones clear, budgets, sign inventory). ALL GREEN.
- `tools/build_binary_place.py`: the world's SpawnLocation now rides at
  Workspace top level (HealthService requires top-level); merger stub
  removed. Place self-verify still green (48 PROPs verbatim, 9 markers).
- Full gate green: syntax 93, refs, waits 262/28, sim `WARNS:0 ERRORS:0` x2.
