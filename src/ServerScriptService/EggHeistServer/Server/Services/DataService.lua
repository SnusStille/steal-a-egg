-- EggHeist | Server/Services/DataService.lua
...
local function loadProfile(player)
	...
	local loaded = nil
	if storeOk and not RunService:IsStudio() then
		local ok, result = pcall(function()
	...
	elseif storeOk and RunService:IsStudio() then
		-- Still try in Studio (works when API access is enabled)
		local ok, result = pcall(function()
			return store:GetAsync(key)
		end)
		if ok then
