...
| `validate_syntax.py` | `python3 tools/validate_syntax.py` | Parses all 84 `.luau` files with luaparser. Needs `pip install luaparser`. |
...
| `sim_boot.py` + `sim_stub.luau` | `python3 tools/sim_boot.py` | Headless integration run of the REAL built game (stubbed engine): server boot → client boot → 2-player join/claim → economy loop → trade → heists → events. Needs `pip install lupa`. Expect `WARNS:0 ERRORS:0`. |
