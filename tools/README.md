...
| `sim_boot.py` + `sim_stub.luau` | `python3 tools/sim_boot.py` | Headless integration run of the REAL built game (stubbed engine): server boot → client boot → 2-player join/claim → place/hatch economy → trade → egg-snatch → heists → events. Needs `pip install lupa`. Expect `WARNS:0 ERRORS:0`. |
